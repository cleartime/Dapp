...
kitSys.prototype = {
    ...
    add_kit_to_list: function(text) {
        ...
        var result = this.query_kit_by_key(obj.from + "_" + obj.time_stamp);
        if (result.success) {
            ...
            kit.time = obj.time;
            kit.from = obj.from;
            kit.kitKey = kit.from + "_" + obj.time_stamp;
            var index = this.kit_list_size;
            this.kit_list_array.put(index, kit.from + "_" + obj.time_stamp);
            this.kit_list.put(kit.from + "_" + obj.time_stamp, kit);
    ...
    add_comment_to_list: function(text) {
        ...
        obj.commentKey = obj.from + "_" + obj.time_stamp;
        ...
        comment.commentKey = obj.commentKey;

        var index = this.commentItem_list_size;
        this.commentItem_list_array.put(index, obj.commentKey);
        this.commentItem_list.put(obj.commentKey, comment);
            ...
            if (searchKey) {
                if (kit) {
                    if ((kit.topic.indexOf(searchKey) != -1) || (kit.hotel.indexOf(searchKey) != -1) || (kit.place.indexOf(searchKey) != -1)) {
                        ...
                        if (kit.kit_text.length < 180) {
                            temp.kit_text = kit.kit_text;
                        } else {
                            // 截断处理
                            temp.kit_text = kit.kit_text.substring(0, 180);
                            temp.kit_text = temp.kit_text + "...";
                        }
                        dataList.push(temp);
                    }
                }
    ...
    query_comment_by_page: function(keys, page) {
        ...
        for (var i = 0; i < number; i++) {
            ...
            if (comment && comment.kitKey == keys) {
    ...
    query_comment_sum: function(from) {
        ...
        for (var i = 0; i < number; i++) {
            key = this.commentItem_list_array.get(i);
            comment = this.commentItem_list.get(key);
            if (comment && comment.kitKey == from) {
                num++;
            }
    ...
    rewardOther: function(info) {
        ...
        var kit = this.kit_list.get(info.key);
        if (kit) {
